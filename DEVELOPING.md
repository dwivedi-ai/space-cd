...
| `XO_INTELLIGENCE_RECALIBRATE` | recalibrate a new session's decision from past sessions of the same kind on the same setup (labels as in `scripts/intelligence_report.py`; 5+ sessions, 60%+ under- or over-powered → one step along the profiles' `tiers`): `off` (default), `shadow` (log a `correction` on the decision line, apply nothing), `on` (same as `shadow` for now). Reads the local decision logs only | unset → `off` | unset → `off` | `services/cowork_agent/intelligence/mode.py`, `recalibrate.py` |
